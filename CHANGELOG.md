...
- SKILL.md: added update.sh to Available Scripts table and Recovery table
- README.md: added "Update coding tools only" section with examples

## [1.4.8] - 2026-08-05

### Changed

- **Removed auto-update check from bootstrap.sh** — was redundant on fresh
  installs (all tools already latest). Replaced with a one-line hint in
  the summary: `To check for coding tool updates later: ./scripts/update.sh`
- **Grouped update.sh components into two categories:**
  - Coding Tools: opencode, oh-my-opencode-slim, Codex CLI, Claude Code, Pi
  - Infrastructure: LiteLLM, Grafana, Prometheus
- Added descriptive header to update.sh explaining what it does and
  doesn't touch (passwords, API keys, virtual keys are never modified)
- Interactive selection menu now shows grouped sections

### Updated

- SKILL.md: clarified update.sh is standalone, documented grouped layout
- INSTALLATION.md: updated "Updating coding tools" section with groups
- REFERENCE.md: updated update.sh script table entry
...
- **Updated deepseek-v4-flash rate limits** — RPM 3 → 15, TPM 30,000 → 60,000
  (Huawei MaaS revised quotas, making flash more practical for agentic workloads)

### Updated

- scripts/helpers/models.sh: flash RPM/TPM values
- configs/litellm/config.yaml.template: flash tpm/rpm
- REFERENCE.md: model table RPM for deepseek-v4-flash
