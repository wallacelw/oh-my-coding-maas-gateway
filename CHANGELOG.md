...
- **Observer image input: `attachment` → `modalities`** — `attachment: true`
  on the model entry does not enable image input in opencode's read tool;
  the tool checks `capabilities.input.image`, populated only from
  `modalities.input`. Replaced `"attachment": true` with
  `"modalities": { "input": ["text", "image"], "output": ["text"] }` on
  both deepseek-v4.1-flash entries in `opencode.json.template`. Verified
  against opencode source (transform.ts `unsupportedParts` check) and
  confirmed in the v1.18.32 binary. Direct MaaS vision test confirmed
  (model described a complex image correctly through the gateway).
