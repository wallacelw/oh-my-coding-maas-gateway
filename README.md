...
**Telemetry pipeline:** LiteLLM (OpenLit SDK auto-instrumentation) → OTLP → OpenLit → ClickHouse
...
- **GenAI metrics**: ITL (inter-token latency), TTFT (time to first token), TPOT (time per output token), cost per token
