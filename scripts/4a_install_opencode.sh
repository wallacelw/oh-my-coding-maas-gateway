...
#
# Usage:
#   ./4a_install_opencode.sh                          # interactive — prompts for keys
#   ./4a_install_opencode.sh --virtual-key=sk-...     # non-interactive — use existing virtual key
#   ./4a_install_opencode.sh --dry-run                # preview changes without modifying anything
...
if [ -z "$VIRTUAL_KEY" ]; then
  if [ "$DRY_RUN" = true ]; then
    echo "   Would mint key via 3_mint_key.sh --alias=opencode --no-budget --quiet"
    VIRTUAL_KEY="sk-dryrun-placeholder"
  else
    VIRTUAL_KEY=$("$SCRIPT_DIR/3_mint_key.sh" --alias=opencode --no-budget --quiet)
...
if [ "$DRY_RUN" = true ]; then
  ...
  echo "   Template: $PROJECT_DIR/configs/opencode/opencode.json.template"
