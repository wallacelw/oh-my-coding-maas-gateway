...
**Step 1** — Ask user for MaaS API key (from https://console.huaweicloud.com/modelarts/, region ap-southeast-1)

**Step 2** — Validate key: `curl -sf -m 10 "https://maas-api.ap-southeast-1.myhuaweicloud.com/v1/infers" -H "Authorization: Bearer $MAAS_KEY" && echo OK || echo INVALID`

**Step 3** — Ask: "How many additional MaaS keys for load balancing?" If >0, prompt one-by-one.

**Step 4** — Check prerequisites (bun, jq, docker, git, python3). Install missing:

| Tool | Install |
|------|---------|
| bun | `curl -fsSL https://bun.sh/install \| bash` |
| jq | `sudo apt-get install -y jq` or `brew install jq` |
| docker | Manual: https://docs.docker.com/get-docker/ |

**Step 5** — Check Docker running: `docker info >/dev/null 2>&1`

**Step 6** — Check existing install: `[ -d /home/oh-my-litellm-opencode ]` — ask user: update or fresh?

**Step 7** — Check port 4000 available
...
./scripts/1_init_env.sh --auto
./scripts/2_generate_config.sh
docker compose up -d
```

**Step 9** — Wait healthy (up to 90s): `docker compose ps` — both services must show `healthy`

**Step 10** — Install opencode: `./scripts/3_install.sh` (installs opencode + plugin + companion + mints/reuses virtual key)

**Step 11** — Validate: `./scripts/5_validate.sh`
...
⚠️  If keys were visible in terminal, rotate them:
  1. Get new key from MaaS console
  2. Edit .env: replace HUAWEI_MAAS_API_KEY
  3. Run: ./scripts/2_generate_config.sh && docker compose restart litellm
  4. Run: ./scripts/5_validate.sh
```
